...
- **RF-37 — Paginación del listado admin de solicitudes:** `GET /api/verificaciones/admin` acepta `page` (default 1, ≥ 1) y `limit` (default 10, rango 1..50) como query params, además del filtro `estado` existente. Responde `{ data: { solicitudes, total, pagina, tamanioPagina, totalPaginas } }`, con `total` sin paginar, orden `createdAt DESC` y `totalPaginas ≥ 1`. El panel `/admin/verificaciones` mantiene el filtro por estado en la URL y renderiza controles Anterior/Siguiente y "Página X de Y" (patrón `/listados`), conservando el estado vacío (`EstadoVacio`) cuando `total = 0`. Parámetros inválidos (`limit` fuera de rango o `page` no numérico) responden 400 `CUERPO_INVALIDO`.
- **RF-38 — Sello de vendedor verificado en tarjetas de publicaciones:** los listados de publicaciones (búsqueda `/listados`, recientes del inicio, favoritos y publicaciones del perfil público) muestran el sello Verificado (`BadgeVerificado`) en la tarjeta de cada publicación cuyo dueño esté `VERIFIED`. Los queries públicos de publicaciones exponen `sellerVerified` del dueño; la tarjeta `TarjetaPublicacion` recibe y renderiza el sello, y los vendedores `NONE`/`PENDING`/`REJECTED` no muestran sello. La verificación NO altera el orden ni los filtros de la búsqueda (sin ranking).
