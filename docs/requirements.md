...
- **RF-26 — Registro de compra transaccional:** al comprar una publicación, el sistema crea un registro `Compra` (comprador, publicación, precio unitario, moneda, cantidad 1 y fecha) en la misma transacción que el decremento atómico de stock, de modo que si la transacción falla no hay decremento ni registro. El endpoint `POST /api/listings/[id]/comprar` devuelve `compraId` dentro de `{data}`, y el botón de compra consume el nuevo contrato.
- **RF-27 — Calificación de venta:** solo el comprador de una `Compra` puede calificar la venta, una sola vez por compra y dentro de los 30 días posteriores a la compra. La calificación usa un puntaje entero de 1 a 5 y un comentario opcional de hasta 500 caracteres. Al calificar, el sistema recalcula de forma atómica el promedio y la cantidad de calificaciones del vendedor (`ratingAvg`/`ratingCount`) y notifica al vendedor, sin que un fallo de notificación revierta la calificación.
- **RF-28 — API de calificaciones tipada:** el endpoint `POST /api/ratings` exige sesión, valida el cuerpo con Zod (`compraId` uuid, `puntaje` entero 1-5, `comentario` opcional de hasta 500 caracteres) y responde errores tipados: 401 `NO_AUTENTICADO`, 404 `COMPRA_NO_ENCONTRADA`, 403 `SIN_PERMISO`, 409 `YA_CALIFICADA`, 410 `VENTANA_EXPIRADA`, 422 `VALIDACION` y 500 `ERROR_INTERNO`.
- **RF-29 — "Mis compras" y formulario de reseña:** la página `/compras` (solo con sesión; sin sesión redirige a login) lista las compras del usuario autenticado y muestra el botón "Calificar" únicamente para compras dentro de la ventana de 30 días y sin calificación previa. El formulario usa estrellas accesibles (teclado y etiquetas), valida en servidor vía RF-28 y cubre los estados de carga, error, éxito y vacío. La interfaz está en español y es responsive.
